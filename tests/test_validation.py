...
class TestValidationService(unittest.TestCase):
    ...
    def test_evaluate_verdict_missing_fields(self):
        ...
        self.assertTrue(any("missing critical fields" in anomaly for anomaly in result.reasons))
        self.assertTrue(any("low OCR confidence" in anomaly for anomaly in result.reasons))
        self.assertTrue(any("validation score too low" in anomaly for anomaly in result.reasons))
    ...
    def test_evaluate_verdict_quality(self):
        ...
        validation = compute_validation_score(fields, blocks, doc_type="license") # Score 80
        result = evaluate_verdict(blocks, fields, validation, doc_type="license")
