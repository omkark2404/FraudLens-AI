...
def evaluate():
    ...
    for filename in files:
        ...
        try:
            ...
            print(f"{filename}: Expected={expected_valid}, Got={verdict} (Reasons: {getattr(res.verdict, 'reasons', [])}) -> Correct={correct}")
            print(res.fields)
    ...
    tp = sum(1 for r in results if not r["expected_valid"] and not r["correct"] == False) # wait, logic is easier:
    
    tp = sum(1 for r in results if not r["expected_valid"] and r["verdict"] in ["rejected", "needs_review", "needs_better_image"])
    fp = sum(1 for r in results if r["expected_valid"] and r["verdict"] in ["rejected", "needs_review", "needs_better_image"])
    fn = sum(1 for r in results if not r["expected_valid"] and r["verdict"] == "no_anomalies_detected")
    tn = sum(1 for r in results if r["expected_valid"] and r["verdict"] == "no_anomalies_detected")
